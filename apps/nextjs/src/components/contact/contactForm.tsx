  ...
  return (
    <div>
      <form className="flex w-full flex-col" onSubmit={handleSubmit(onSubmit)}>
        ...
        <input
          id="email"
          className={`mb-2 rounded border-2 bg-white/10 p-2 ${
        ...
        {errors.email?.message && (
          <FormError className="mb-5">{errors.email.message}</FormError>
        )}
        ...
        />
        {errors.body?.message && (
          <FormError className="mb-5">{errors.body.message}</FormError>
        )}
        <Button type="submit" className="ui_btn" disabled={isSubmitting}>
          お問い合わせを送信する
        </Button>
        {isSubmitSuccessful && (
          <FormSuccess className="mt-5 p-3">
            お問い合わせありがとうございました。
          </FormSuccess>
        )}
      </form>
