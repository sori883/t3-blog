...
export async function PostInCategoryList(props: {
  slug: string;
  page: string | undefined;
}) {
  const { limit, offset } = makeLimitOffset(props.page);
  const { posts, totalCount } = await api.post.postsInCategory.query({
    slug: props.slug,
    limit,
    offset,
  });
  ...
  return (
    <div>
      <div className="mb-12 grid grid-cols-2 gap-x-4 gap-y-12">
        {posts.map((p) => (
